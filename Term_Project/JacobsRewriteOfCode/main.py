from Tenant import Tenant # Tenant class
from Tenant_List import Tenant_List # Tenant_List class
from USER import User # User Class
from User_List import User_List # User_List class 
from Expense import Expense # Expense Class
from Expense_List import Expense_List # Expense_List class
from Rent_Row import Rent_Row #rent_row class
from Rent_List import Rent_List # rent_list class
...
class UI:  #user interface
    ...
    def input_screen(self, scanner_2):
        if scanner_2 == 't':
            tenant = Tenant()
            ten_list = Tenant_List(self.__tenants_list[self.__loged_user_idx],
                                   self.__rent_records[self.__loged_user_idx])
            ten_list.add_nu_tenant(tenant.get_Tenant(), tenant.get_Apt())
        elif scanner_2 == 'r':
            rent_row = Rent_Row()
            rent_list = Rent_List(self.__rent_records, self.__loged_user_idx,
